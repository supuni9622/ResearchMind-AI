...
**Latest re-audit:** 2026-07-19 (Chat history/reliability and title-generation follow-up; this delta also corrects stale evaluation and Research-memory findings in earlier sections)
...
1. **Memory extraction is not fully best-effort.** Individual memory writes are caught, but `memory_extraction_service.extract()` itself is awaited outside a `try` block. An extraction outage after the turn commit can interrupt the remaining completion tail (title/artifact handling) and produce a server-side error after tokens have already been sent. Wrap extraction independently and log it as a non-fatal post-processing failure.
2. **History is silently truncated and unpageable.** Both message replay/transcript loading and conversation listing use a fixed limit of 50 without a cursor, total count, or UI indicator. Long threads lose older context from the model and cannot be fully reviewed from the API. Add cursor pagination and a deliberate context-window strategy (summary/compaction) rather than silently dropping history.
...
5. **Per-user generation cost now has a durable source of truth.** `generation_usage` is an append-only PostgreSQL ledger keyed by `request_id`, recording owner, generation/conversation/session IDs, provider/model/runtime, token counts, estimated USD cost, streaming and cache state. Both Generation and Streaming services write it after successful completion; cache replays are recorded at zero new-provider cost. `GET /usage/summary` is owner-scoped and drives the dashboard’s current-month/all-time cost card. Historical usage intentionally remains unavailable: it was never persisted with an owner-linked cost. The remaining accounting limitation is that streamed token usage is locally estimated where providers do not expose usage metadata, so this is suitable for estimated spend management—not invoice reconciliation.
...
**Composite: ~3.15/5 → ~3.45/5 (estimate)** — this follow-up closes real Chat product/reliability gaps, corrects the prior Research-memory and evaluation findings, and adds an account-backed conversation surface. The largest open question is unchanged: Chat still does not do query rewriting or RAG, while operational durability/cost/pagination gaps remain (§0.0).
...
12. **Harden Chat post-processing and growth controls.** Title creation is now lease-based and one-time; isolate memory-extraction failures, add cursor pagination plus context compaction, and decide how to represent interrupted streams. These are current reliability/scale loopholes in an otherwise working Chat history flow (§0.0).
