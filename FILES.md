...
|------|-------------|
| `__init__.py` | Package marker |
| `types.py` | Minimal framework-independent contracts: `ResearchRuntimeStatus` (Phase 1 skeleton), `ResearchRunStatus` (the real lifecycle enum — created/planning/researching/reviewing/synthesizing/paused/awaiting_approval/completed/completed_with_limitations/cancelled/failed), `TERMINAL_RESEARCH_RUN_STATUSES`, `ResearchProposalStatus`, `ResearchRunDispatchStatus`, `ResearchRuntimeRequest` |
...
| `lifecycle.py` | `transition_run()` — validated `ResearchRunStatus` state-machine transitions (an explicit allowed-transitions table), structured-logged on every transition and rejection. `RESEARCHING → AWAITING_APPROVAL` added 2026-07-23 for the report-approval pause (`AWAITING_APPROVAL → RESEARCHING` already existed for the earlier crash-resume case, reused for report-decision resume too) |
...
| `chat.py` | **Implemented** — `ChatStreamRequest` plus owner-scoped conversation-summary and conversation-detail response schemas for server-backed chat history |
...
| `src/features/research/types.ts` | `ResearchTurn`/`ResearchStage`/`ResearchConversationEntry`; re-exports `PROVIDER_OPTIONS` from `lib/api.ts` (moved there in Phase 4.2 since it's runtime-agnostic, shared with Chat). **2026-07-23, later**: `ResearchMode` (`'linear' | 'deep'`), `DeepResearchStage` (`plan_review → running → report_review → done/failed`, plus `error`), `DeepResearchProgressEvent`, `DeepResearchTurn`; re-exports the Deep Research response-shape types (`DeepResearchPlan`, `DeepResearchProposal`, `DeepResearchRun`, `ResearchEscalationCheck`, ...) from `lib/api.ts`, which owns them (matches the file's own existing convention of defining backend-mirroring shapes in `api.ts`, not here) |
...
| `src/features/research/components/*.tsx` | `citation-card`, `empty-workspace`, `research-block` (citation-annotated answer card), `research-composer` (provider picker + Linear/Deep mode toggle, added 2026-07-23 later), `research-sidebar` (history list), `source-card`, `source-panel` (citations/passages/metrics), `streaming-status`, `deep-research-block` (**new, 2026-07-23 later** — renders all `DeepResearchStage`s: plan review with a complexity/budget badge, a live SSE-fed progress list, report-approval with an optional reject reason, and a completed card with a PDF link), `escalation-suggestion` (**new, 2026-07-23 later** — the accept/reject banner shown when `checkEscalation()` suggests Deep Research) |
| `src/features/chat/types.ts` | (new, Phase 4.2) `ChatMessage`/`ChatMessageStage`/`ChatConversation`/`ChatConversationSummary` |
| `src/features/chat/use-chat.ts` | `useChat()` — drives streaming state and uses `GET /chat/conversations` plus `GET /chat/conversations/{id}` as the source of truth for conversation summaries and replay. It retains only immediate in-flight UI state locally. |
| `src/features/chat/components/*.tsx` | `chat-sidebar` (server-synced list; full generated title exposed via native tooltip and accessible label), `message-bubble` (ChatGPT-style bubbles), `chat-composer` (provider picker), `empty-chat` (suggested prompts) |
| `src/hooks/use-auth.tsx` | `AuthContext` — token storage, profile fetch, `isUnauthorized` state |
| `src/lib/api.ts` | Typed API client — shared stream models and `api.research.{ask,stream,get,listConversations,getConversation}`, `api.chat.{stream,listConversations,getConversation}`, and document operations. **2026-07-23, later**: Deep Research response-shape types (`DeepResearchPlan`/`DeepResearchProposal`/`DeepResearchRun`/`ResearchEscalationCheck`/...) plus `api.research.{checkEscalation,createProposal,approveProposal,getRun,cancelRun,submitReportDecision,getReportDownload,streamRunEvents}` — `streamRunEvents` is a `fetch`+SSE generator (same auth-header workaround as `stream()`), the only one of these that takes an `AbortSignal` for long-lived-connection cleanup |
