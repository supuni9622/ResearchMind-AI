...
| `prds/` | New subdirectory holding duplicate copies of most `*_platform_prd.md` files also still present at root (e.g. `artifacts_platform_prd.md`, `memory_platform_prd.md`) plus two new PRDs not present anywhere else (`1.researchmind_mcp_integration_prd.md`, `2. web_search_tool_platform_prd.md`). Root copies are unchanged/still canonical per this file's existing entries below; the duplication's purpose wasn't part of this session's work — noted here so it isn't missed, not resolved |
...
| `research/models.py` | `ResearchEventType` — **live as of 2026-07-23**, emitted throughout the Research Runtime Platform (`ai/runtime/research/`) via `LangGraphResearchEventAdapter` and `ResearchRuntimeEventJournal`; no longer reserved-but-unused |
| `chat/models.py` | `ChatEventType` — **new, 2026-07-25 (ADR-036 addendum)**: `chat_web_search_started/completed/skipped`, emitted with `category=EventCategory.TOOL` by `runtime/chat/web_search.py::run_chat_web_search()`, chained ahead of the normal token stream via `api/v1/chat.py`'s `_chain_events()` wrapper generator |
...
##### `ai/runtime/chat/` — **Implemented** (new, 2026-07-25, ADR-036 addendum)
