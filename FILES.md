...
| `v1/chat.py` | **Implemented** (Streaming Platform, Milestone 2.9.10; Artifact Platform, Milestone 3.10; Memory Platform wiring, 2026-07-18) — `POST /chat/stream` (SSE; `get_current_user` auth, since this is a `POST` consumed via `fetch`/`ReadableStream` on the frontend rather than a bare `EventSource`, which can't attach a custom `Authorization` header) and `WebSocket /chat/ws` (`?token=` query-param auth, since a browser's WS handshake has the same header limitation). Both load/create a `Conversation` via `ConversationService`, fold prior turns into a transcript-prefixed `user_prompt`, call `StreamingService.stream_generate()`, and persist the completed turn once the stream reaches `COMPLETE` (`_persist_on_complete` — accumulation/persistence lives at the route level, not inside the generic Streaming Platform, keeping that platform conversation-agnostic). Also best-effort persists a `ConversationIdentity` (once, via `_persist_conversation_identity`) and a `ConversationTurnArtifact` per completed turn through `ConversationArtifactWriter`, policy-gated by `ArtifactPolicyService`. **Now also wired to the Memory Platform** (`ai/memory/`), mirroring `ResearchService`'s Runtime Memory Injection: `_retrieve_memory_context()`/`_build_request()` fetch `MemoryService.get_context()` (session id = `conversation_id`) and prepend it to `PromptContext.context` via `with_memory_context()`; `_extract_and_store_memory()`, called from `_persist_on_complete()` right after the turn is appended, stores the raw turn as SESSION memory and runs `MemoryExtractionService.extract()` for USER/RESEARCH candidates. `/chat/stream` gets both services via `Depends(get_memory_service)`/`Depends(get_memory_extraction_service)`; `/chat/ws` uses the plain-Python `build_memory_service(session)`/`build_memory_extraction_service()` factories instead, since that route manages its own `AsyncSession` outside FastAPI's DI graph. This wiring also surfaced and fixed two Generation Platform bugs — see `ai/runtime/generation/` (root files) above. Test fakes for both services live in `tests/integration/ai/test_chat_stream.py` (`_FakeMemoryService`/`_FakeMemoryExtractionService`). **`_build_request()` now also sets `GenerationRequest.session_id=conversation_id`** (Phase 4.2, Chat Surface Frontend Integration) — previously only `conversation_id` was set, so `StreamEvent.session_id` was always `null` and a frontend starting a brand-new conversation had no way to learn the server-minted `conversation_id` after turn 1 (`ConversationService.get_or_create()` 404s on an unrecognized client-supplied id rather than creating one). Mirrors `ResearchService`'s identical `session_id=research_id` pattern; verified against the existing 3 tests in `test_chat_stream.py`, still passing |
...
| `src/app/(app)/research/page.tsx` | **Research** page — "user knows the goal" surface: sidebar + citation-card blocks + source/citation panel, wired to the live `/research`, `/research/stream` (SSE), `GET /research/{id}` APIs via `features/research/use-research.ts` |
| `src/app/(app)/chat/page.tsx` | **Chat** page (new, Phase 4.2) — "user knows the question" surface: conversation sidebar + ChatGPT-style message bubbles + composer, wired to the live `POST /api/v1/chat/stream` (SSE) via `features/chat/use-chat.ts`. Deliberately no citations/sources panel (Chat has no retrieval/RAG wired server-side yet) |
...
| `src/features/research/types.ts` | `ResearchTurn`/`ResearchStage`/`ResearchHistoryEntry`; re-exports `PROVIDER_OPTIONS` from `lib/api.ts` (moved there in Phase 4.2 since it's runtime-agnostic, shared with Chat) |
| `src/features/research/use-research.ts` | `useResearch()` — drives per-turn state (`searching`→`generating`→`done`/`error`) against `api.research.stream()`; history persists client-side in `localStorage` keyed by `research_id`, replayed via `GET /research/{id}` (server is the source of truth — only the id needs to survive a refresh) |
| `src/features/research/components/*.tsx` | `citation-card`, `empty-workspace`, `research-block` (citation-annotated answer card), `research-composer` (provider picker), `research-sidebar` (history list), `source-card`, `source-panel` (citations/passages/metrics), `streaming-status` |
| `src/features/chat/types.ts` | (new, Phase 4.2) `ChatMessage`/`ChatMessageStage`/`ChatConversation`/`ChatConversationSummary` |
| `src/features/chat/use-chat.ts` | (new, Phase 4.2) `useChat()` — drives multi-turn message state against `api.chat.stream()`. Unlike `useResearch`, persists **full** `ChatConversation` objects (every message, not just an id) to `localStorage` (`rm_chat_conversations`, capped at 30) and rehydrates directly from there on conversation switch — there is no `GET /chat/conversations`/`GET /chat/{id}` endpoint for it to replay from instead |
| `src/features/chat/components/*.tsx` | (new, Phase 4.2) `chat-sidebar` (conversation list, per-item delete), `message-bubble` (ChatGPT-style user/assistant bubbles — deliberately distinct visual language from Research's citation-card blocks), `chat-composer` (provider picker, mirrors `research-composer`), `empty-chat` (suggested-prompt empty state) |
| `src/hooks/use-auth.tsx` | `AuthContext` — token storage, profile fetch, `isUnauthorized` state |
| `src/lib/api.ts` | Typed API client — `UserProfile`, `Document`, `GenerationProvider`, `PROVIDER_OPTIONS` (moved here in Phase 4.2), `Citation`, `ResearchSource`, `RuntimeStreamEvent` (canonical `StreamEvent` shape shared by Research and Chat; `ResearchStreamEvent` kept as a type alias), `api.research.{ask,stream,get}`, `api.chat.stream` (new, Phase 4.2 — SSE POST to `/api/v1/chat/stream`, no non-streaming or GET counterpart exists), `api.documents.{list,upload}` |
