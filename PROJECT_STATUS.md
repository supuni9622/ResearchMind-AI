...
**Last Updated:** 2026-07-18 (+ Chat Surface Frontend Integration — a new `/chat` page in `apps/web`, plus the `GenerationRequest.session_id` backend fix it depended on; Memory Platform built and wired into both Research and Chat, two Generation Platform bugs fixed, Routing Platform AUTO default changed to Groq — see "Recently Completed" below)
...
- `features/chat/{types.ts,use-chat.ts,components/}` — new feature. `use-chat.ts` deliberately does **not** mirror `use-research.ts`'s replay-from-server pattern: there is no `GET /chat/conversations` or `GET /chat/{id}` endpoint (Chat is stream-only, no history read path). So it persists full `ChatConversation` objects — every message, not just an id — to `localStorage` (`rm_chat_conversations`, capped at 30); switching conversations rehydrates directly from localStorage rather than re-fetching. This is a documented, deliberate gap, not an oversight — see "Not Yet Built" below.
- `app/(app)/chat/page.tsx` — new "AI Assistant" page: a conversation-list sidebar (client-side only) + ChatGPT-style message bubbles (user right-aligned, sage-tinted; assistant left-aligned, plain) + a composer with the same provider picker Research uses. Deliberately has **no** source/citation panel — Chat still has no retrieval/RAG wired server-side (`chat.py` always builds `PromptContext(context="", chunks=[])`, per Milestone 12's write-up), so there's nothing to show there; the empty state explicitly directs users to Research for cited, document-grounded answers instead of faking an empty panel.
...
- ❌ `GET /chat/conversations` / `GET /chat/{id}` — no server-side history read path exists; `use-chat.ts`'s localStorage-as-source-of-truth is a workaround, not the intended long-term shape. If this is ever added, `use-chat.ts` can be simplified to match `use-research.ts`'s server-replay pattern exactly.
...
`tsc --noEmit` clean, `next build` clean (no lint warnings after fixing one `react-hooks/exhaustive-deps` warning), all 44 chat+research backend pytest tests pass. Did **not** verify via live Cognito login or a real LLM provider call, per this project's standing convention of never verifying against live LLM/auth infrastructure.
...
- ❌ No dedicated automated tests for the Memory Platform's own services yet (`tests/integration/test_memory.py` is still an empty stub) — today's coverage is indirect, through the Chat/Research integration points
...
| Phase 4.2 — Chat Surface Frontend Integration | ✅ Complete — new `apps/web` `/chat` page wired to the live `POST /api/v1/chat/stream`, separate from Research per the Chat-vs-Research product IA; required fixing a backend gap (`GenerationRequest.session_id` never set in `chat.py`, blocking multi-turn chat from a fresh session) |

---

# Recently Completed

✅ Chat Surface Frontend Integration (Phase 4.2) — `apps/web` got a new `/chat` page, separate from Research, implementing the Chat-vs-Research product split (Chat: user knows the question, fast/interactive/no-citations; Research: user knows the goal, slow/cited/report-generating). Required one small backend fix first: `chat.py` never set `GenerationRequest.session_id`, so a client starting a new conversation had no way to learn the server-assigned `conversation_id` from the SSE stream (the server must mint it — `ConversationService.get_or_create()` 404s on an unrecognized client-supplied id rather than creating one) — permanently blocking multi-turn chat from a fresh session. Fixed with `session_id=conversation_id`, the same pattern `ResearchService` already used. New frontend: `features/chat/` (types, `use-chat.ts`, ChatGPT-style bubble components), `app/(app)/chat/page.tsx`, a new "Chat" nav entry, `api.chat.stream()` in `lib/api.ts`. Since there's no `GET /chat/{id}` endpoint, conversation history is persisted client-side in full (not just an id, unlike Research) — a documented gap, not an oversight. `tsc --noEmit`/`next build` clean, all 44 chat+research backend tests pass. See Phase 4.2 above for full detail.
...
The design question Phase 4.1 deferred — separate Chat nav entry/page vs. a unified mode-toggle input — is now settled: Chat got its own `/chat` page in `apps/web`, wired to the previously-frontend-less `POST /api/v1/chat/stream`. Required fixing a real backend gap first (`GenerationRequest.session_id` never populated in `chat.py`, so a new conversation's server-assigned id could never reach the client) — see Phase 4.2 above for the full write-up, including the deliberate scope decisions (no citations panel, since Chat still has no retrieval wired; localStorage-based history, since there's no `GET /chat/{id}` endpoint).
...
Phase 4.2 — Chat Surface Frontend Integration ✅ — apps/web's new /chat page wired to the live Chat API
  ...
  GET /chat/conversations, /chat/{id} ❌ (no server-side history read path — localStorage workaround in use-chat.ts)
...
Chat 🟡 — basic streaming chat (SSE + WebSocket, Conversation/Message history) is live via the Streaming Platform (Milestone 2.9.10), and now wired to the Memory Platform (Milestone 12 ✅ — session + user + research memory injected/extracted every turn); still not wired to Retrieval/Context (no `PromptContext.chunks`), so the now-guardrail-aware `ContextBuilderService` still isn't reachable from this path — see `guardrail_integration_prd.md`'s "Retrieval Integration" note. Frontend surface now live too (Phase 4.2 ✅ — a separate `/chat` page, not a mode toggle on Research)
...
Milestone 12 — Memory Platform ✅ — per `memory_platform_prd.md`
  ...
  LLM-driven extraction (MemoryExtractionService) ✅
