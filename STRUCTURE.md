...
│   │       │   │       ├── validation/             # ~80% Implemented — input/output/hallucination/runtime validators, registry, scoring, ValidationReport
│   │       │   │       │   ├── service.py          # ValidationService — validate_input()/validate_output()/validate_hallucination()/validate_runtime()/validate(); crashing validator → WARNING not a hard failure
...
│   │       │   │       │   ├── create.py           # create_validation_registry() / get_validation_service() — registers all input+output+hallucination validators and the ResearchRuntimeContract
│   │       │   │       │   ├── output/
...
│   │       │   │       │   └── runtime/            # Runtime Validation Platform (per runtime_validation_prd.md) — 4th ValidationStage.RUNTIME stage; only ResearchRuntimeContract implemented, planner/reviewer/agent/mcp are future work
...
│   │       │   │       │       │   ├── consistency.py      # ConsistencyValidator — evidence.section_id must reference an existing section (orphan reference check)
...
│   │       │   │       │           └── research.py     # ResearchRuntimeContract — summary/≥2 sections/≥1 citation/≥1 evidence/confidence in [0,1], entirely composed from validators/
...
│   │   │       └── runtime/                 # Runtime Validation Platform tests (new, 109 cases)
...
│   │   │           ├── validators/              # test_completeness.py, test_consistency.py, test_confidence.py, test_evidence.py, test_citation.py — one file per generic validator
│   │   │           └── contracts/test_research.py  # ResearchRuntimeContract — compliant output valid, PRD §3's trivial-output example fails every requirement, issues tagged with contract_name
...
├── LICENSE
├── phase-3-ai-runtime-roadmap.md  # Frozen v2.0 — Retrieval, Context, Generation & Research Runtime roadmap (Phase 3.4–3.12), progress tracked inline — Generation Platform (3.8) ~75% complete
...
├── ResearchMind-Roadmap-v2.md   # AI Engineering Roadmap v2 — 10-phase vision, frozen tech decisions — AI Runtime Platform (Phase 3) ~75% complete
...
| Generation Platform | `apps/api/app/ai/runtime/generation/` | Owns all LLM interactions over 5 providers (Groq, OpenAI, Claude, Gemini, Ollama): native structured-output decoding, a parser/repair fallback, input/output/hallucination/runtime Validation Platform integration (registry, weighted scoring, `ValidationReport`, `ResearchRuntimeContract`), a regenerate-on-invalid-output loop, a Prompt Platform bridge (`generate_from_template()`), a Routing Platform bridge — `generate()`/`stream_generate()` resolve a model via `routing_strategy` when no `provider` is given explicitly — a Runtime Caching Platform bridge, a Streaming Platform bridge, and an Artifact Platform bridge (`generate()` persists a `GenerationArtifact`). ~85% complete — see `docs/architecture/structured-output-platform.md`; now reachable over HTTP via `POST /api/v1/chat/stream` / `/api/v1/chat/ws` (`/research` still does not exist) |
