...
class EvalScore(Base):
    """
    One row per (generation, metric, source) -- e.g. a single generation
    scored online produces one row for `citation_validity` and, when
    sampled for judge scoring, one more per Ragas metric
    (`answer_relevancy`, `faithfulness`, ...).

    `generation_id` intentionally has no foreign key to
    `generation_usage.generation_id`, matching `Feedback`'s and
    `GenerationUsage.conversation_id`/`session_id`'s established pattern:
    that column isn't unique (`request_id` is), so it can't be an FK
    target.
    """
    ...
    """
    dataset_example_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
