...
export default function MemoryPage() {
  ...
  const canLoad = scope !== 'project' || Boolean(activeProjectId);
  ...
  async function moveMemory() {
    ...
    const destination = moveDestination === 'personal'
      ...
      : moveDestination === 'global'
        ? { scope_type: 'global' as const, project_id: null }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-8 sm:py-10">
      ...
      <div className="mb-6 inline-flex rounded-xl border border-ink-600 bg-ink-900 p-1" role="tablist" aria-label="Memory scope">
        {(['personal', 'global', 'project'] as const).map((item) => <button key={item} role="tab" aria-selected={scope === item} onClick={() => setScope(item)} className={`rounded-lg px-4 py-2 text-[13px] capitalize ${scope === item ? 'bg-sage-800 text-stone-100 shadow-sm' : 'text-stone-500 hover:text-stone-300'}`}>{item} memory</button>)}
      </div>
      ...
      {scope === 'personal' ? (
        ...
        <div className="mb-6 rounded-xl border border-blue-900/70 bg-blue-950/20 px-5 py-4">
          <p className="text-[13px] font-medium text-stone-200">Visible everywhere, always</p>
          <p className="mt-1 max-w-3xl text-[12px] leading-5 text-stone-500">Global memory is injected into every context for you — Personal chats and every project — unlike Personal (which projects only see if you allow inheritance) or Project (isolated to one project).</p>
        </div>
      ) : (
