...
**Latest re-audit:** 2026-07-25/26 (Web Search Tool Platform, ADR-036 — a third human-approval checkpoint reused for Chat's tool-augmented answers too — plus ResearchMind's first MCP **client**, a narrow Research Intelligence MCP paper-search integration, ADR-037; also a real session-state pronoun-resolution production bug fix and a markdown-rendering/citation-badge bug fix. See §0.0.0b)
**Scope:** `apps/api/app/ai/` (Knowledge Platform + Generation Platform + Research API Platform + Research Runtime Platform + Observability Platform + **`app/ai/tools/` (Web Search, Paper Search — new this revision)**), Chat API/conversation persistence, the Chat frontend, and the app-wide infrastructure that supports them (observability, error handling, testing, API wiring).
...
**Post-audit addendum (2026-07-26) — product-scope correction, not a code change:** this report's recurring "P0 finding" (§0.0.0b, §5, and every re-audit delta between 2026-07-18 and 2026-07-26) that `chat.py` hardcodes an empty `PromptContext` and never calls retrieval/`ContextBuilderService` is **factually accurate but was mis-classified as a defect**. It was confirmed with the user (2026-07-26) that Chat's lack of document grounding is the deliberate three-surface product design (Chat fast/conversational/NOT document-grounded; Linear Research and Deep Research document-grounded) — not an unfinished P0 gap. Every prior mention of this finding in the sections below is left as-written since this is a historical audit log, not a living doc, but it should now be read as **a scope note describing an intentional product boundary**, not an open defect requiring a fix. See `PRODUCT_FLOWS_AND_GAPS.md`'s Chat section (updated 2026-07-26) for the corresponding current framing.
...
**Post-audit addendum (2026-07-19 — Memory Platform optimization):** the
original four-memory design was retained and its runtime was optimized rather
than replaced. SESSION now stores compact non-transcript state; PostgreSQL is
still canonical for durable USER/SEMANTIC/RESEARCH records; and Qdrant remains
the semantic/research search index. An owner-scoped availability check skips
durable retrieval for users with no durable memory. Otherwise one query
embedding is shared by concurrent semantic and research searches, with each
branch failing open independently. Extraction is now a versioned, deterministic
final-turn policy behind `MemoryExtractionOrchestrator`, using canonical turn
IDs for idempotency: explicit durable interests are eligible immediately,
generic topics require two distinct sessions and one bounded LLM validation,
and planner/reviewer/tool/helper nodes default to no extraction. Groq remains
the primary extractor with OpenAI fallback. Structured decision/failure/latency
events and the owner-scoped `generation_usage` ledger distinguish answer cost
from `memory_extraction` cost. Focused Memory tests (19) plus Chat/Research
...
## 0.0.0b Re-Audit Delta (2026-07-25/26 — Web Search Tool Platform, Research Intelligence MCP Paper Search Platform, session-state fix, markdown rendering)

Two new external-tool integrations landed since the 2026-07-24 delta (§0.0.0a's item 27), plus two smaller but real bug fixes. Verified by direct code read, the new tests each change added, and — continuing this audit's recurring theme (§0, §0.0.0a) — **production/live-use signal that a green test suite alone did not surface**, three separate times in this delta alone.

**1. Web Search Tool Platform (ADR-036) — a third human-approval checkpoint, reused for Chat's tool-augmented answers with no checkpoint at all.** `app/ai/tools/web_search/` (Tavily-backed) is a new, framework-independent platform reused by two runtimes: Deep Research gets a genuine third `interrupt()` checkpoint (`await_web_search_approval`, reusing the existing bounded gap-research loop rather than a parallel node family — `AUTO` asks unless pre-authorized, `REQUIRED` never asks); Chat gets the same underlying service with **no** checkpoint at all — a single `web_search_enabled` toggle *is* the approval, once, per turn, since Chat has no interrupt/resume mechanism to pause on. Three real, live-found issues surfaced while shipping this, consistent with this audit's running finding that this codebase's test suite is thorough but does not substitute for checking behavior against reality:
   - **A topically-mismatched private corpus passed synthesis and review cleanly.** A real run against a knowledge base containing only a mental-health PDF, asked about climate change, produced a confidently-wrong report — the post-review gap-detection loop only fires *after* a full synthesis pass, and citation-integrity review (which checks whether citations are valid and cover the claims made) has no way to notice the underlying corpus is the wrong subject entirely. Fixed by routing through the necessity-decision checkpoint *unconditionally*, right after evidence aggregation, whenever `auto`/`required` mode is set — catching the mismatch before synthesis is spent on it. A numeric-relevance-score-threshold approach was considered and rejected: `ResearchEvidenceReference.score` is an RRF rank-derived sum, not a semantic-similarity measure, and can't reliably distinguish a confidently-wrong top hit from a genuinely relevant one.
   - **The necessity-decision model silently failed AUTO mode closed, every time.** Production logs showed `gpt-5-nano` (the original default for this one structured-output call) returning genuinely invalid, non-repairable JSON twice in a row — not truncation, a contract-following failure — making AUTO mode appear completely broken end-to-end while every unit test (which uses fakes, not the real model) kept passing. Swapped to `gpt-5-mini`, this app's already-proven default elsewhere, paired with a stricter JSON-only system prompt.
   - **A citation-badge regex only ever matched document citations.** `research-block.tsx`'s `CITATION_TOKEN` regex was `S\d+`-only; a web citation marker (`[W1-1]`) embedded in a rejected report's fallback answer rendered as plain unstyled text instead of a chip — found via live use of the newly-shipped feature, not a design review.

**2. Research Intelligence MCP Paper Search Platform (ADR-037) — ResearchMind's first MCP client, deliberately narrow.** `app/ai/tools/paper_search/` mirrors the Web Search Tool Platform's file layout exactly, but reaches an external "Research Intelligence MCP" server's `search_papers` tool via the official `mcp` SDK's `streamablehttp_client` — a fresh connection per call, no persistent session, optional static bearer-token auth. This is the first MCP client code anywhere in this codebase's history, which matters for how §4.10/§5's "MCP: zero code" finding should now be read (corrected below) — but it is a single hard-coded integration to one server for one tool, not a reusable client abstraction, registry, or manager: **it does not represent progress on the general-purpose MCP Ecosystem this report's Phase 6/7 roadmap items describe**, and a fuller PRD (`prds/1.researchmind_mcp_integration_prd.md` — cached/refreshed JWT service-token auth, retry-with-backoff, a 9-category error taxonomy, the other 5 tools the server exposes) was explicitly confirmed with the user as deferred scope, not an oversight. Chat gets a toggle-only opt-in with deliberately **no** necessity-decision call (simpler than Web Search's AUTO mode, avoiding a second fragile-structured-output surface after the `gpt-5-nano` failure above); Deep Research gets a **non-blocking** `suggest_related_papers` node, sequential between `persist_final_report` and `END` — not a fourth approval checkpoint, since the report is already durably persisted by the time it runs, so nothing is left to gate. Both call sites distill their query through a new `PaperQueryExtractionService` first, after confirming live that a raw Chat prompt and Deep Research's `rewritten_goal` (both full sentences, not topic phrases) return zero results from the paper-search backend — the same class of "raw text doesn't match what an external search API expects" lesson, found the same way, twice in one delta.

**3. A real production bug, unrelated to either tool platform but shipped the same session: session-state pronoun resolution.** A Deep Research follow-up using a pronoun ("so how is magma related to it?") had nothing durable to resolve "it" against — the old `state_from_user_turn` only captured a narrow set of trigger phrases, not ordinary topic drift, so nothing about a session's actual subject had ever been persisted anywhere retrievable. Fixed with a new `SessionStateUpdaterService` (`app/ai/memory/session/state_updater.py`) that asks a cheap model to maintain one evolving, ≤300-character SESSION-memory sentence describing the session's current subject after every turn, upserted in place rather than appended. This is exactly the kind of gap live use surfaces and a unit-test suite structurally cannot: nothing was "wrong" in isolation, the absence only became visible against a real multi-turn conversation.

**4. Markdown rendering across all three product surfaces (uncommitted as of this delta).** Chat, Linear Research, and Deep Research answers now render through a shared `Markdown` component (`react-markdown`+`remark-gfm`+`remark-breaks`) instead of raw `whitespace-pre-wrap` text. Citation badges are now produced by a remark plugin operating on the parsed markdown tree, replacing the hand-rolled string-splitting regex mentioned in finding 1's third bullet — the fix there was a one-line regex patch; this is the actual root-cause fix, since a citation token embedded inside markdown structure (a list item, a bolded phrase) could still slip past a pre-parse string-splitting approach in ways a tree-based plugin cannot.

**What this delta does *not* change.** The report's central, five-cycle-old P0 finding — `chat.py` still builds an empty `PromptContext` and never calls retrieval or `ContextBuilderService` — is completely untouched. This is worth stating explicitly because it would be easy to misread "Chat can now search the web and academic papers" as "Chat now does RAG": it does not. Web Search and Paper Search are both external-tool augmentation, unrelated to and orthogonal to grounding Chat in a user's own uploaded documents, which remains exactly as absent as every prior revision of this report found it.

**Full flow-by-flow reference**: `PRODUCT_FLOWS_AND_GAPS.md` (Chat/Deep Research sections, re-verified against current code alongside this delta) and `ROADMAP.md`'s Phase 5/7 sections; ADR-036 and ADR-037 for the full decision records.
...
The prior audit correctly described Chat as persisted but provider-flattened, yet it predates the product-level history read path and therefore no longer describes the user-visible system accurately. Direct review of `api/v1/chat.py`, `services/conversation.py`, `repositories/conversation.py`, the chat schemas, `use-chat.ts`, and the focused Chat integration tests establishes the following.

**Closed since the prior audit:**

- **Server-backed history is now real.** `GET /api/v1/chat/conversations` returns owner-scoped conversation summaries and `GET /api/v1/chat/conversations/{conversation_id}` returns owner-scoped message replay. `use-chat.ts` now loads these APIs rather than treating browser local storage as the authority. This closes the reload/device-switch failure in the earlier frontend implementation.
- **The stream persistence bug is fixed.** The Chat route recognizes both `CoreEventType.COMPLETE` (`complete`, cache replay) and `StreamEventType.COMPLETED` (`completed`, live providers). Previously a live provider could display an answer while never persisting the turn; current focused tests cover both variants.
- **Replay order is deterministic.** `ConversationService.append_turn()` assigns distinct user/assistant timestamps and the repository has a role-based tie-breaker for legacy tied PostgreSQL transaction timestamps. Replay and transcript order are now user → assistant rather than depending on database tie behavior.
- **Title generation is constrained and one-time.** A best-effort Groq request uses the persisted first user message, with a prompt prohibiting inferred subjects or acronym expansion. A database lease token makes the operation single-writer, releases failed attempts for retry, and prevents later turns from incurring repeat title calls. The sidebar exposes a truncated title with the full value in a native tooltip and accessible label.
- **Knowledge-base dashboard totals are no longer fabricated.** `GET /documents/stats` now counts the authenticated owner's Qdrant vectors by `owner_id`. The dashboard presents this as indexed chunks and embeddings: one current indexed chunk maps to one dense vector, so the two exact counts are equal by design.
- **History growth is now explicit and bounded.** Conversation and message replay use owner-scoped cursor pages (default 50, maximum 100) with UI controls to load earlier records. Before Chat generation, older history is persisted as a deterministic rolling summary while the newest 12 messages remain verbatim; canonical rows are never deleted. This adds no summarization-model call or provider cost. ADR-030 records the decision and tuning basis.

**What remains materially incomplete:** Chat still follows `history → memory context → generation → memory update`; it has **no query-rewriter/condensation step and no document retrieval/context/citation stage**. Transcript history is also still flattened into one `user_prompt`, not passed as native provider messages. Consequently, the product meets basic conversational continuity, but it does not yet meet the requested flow `Conversation Memory → Session Memory → Semantic Memory → Query Rewriter → Retrieval → Generation → Memory Update` end to end.

**Newly identified loopholes / operational risks:**

1. **Historical finding — superseded by the Memory Platform optimization.** Extraction is now invoked through `MemoryExtractionOrchestrator` inside an independent catch-and-log boundary in both Chat and Research, so an extractor/provider failure does not fail the completed answer path. It is also policy-gated and idempotent rather than invoked on every generation. The remaining operational limitation is that eligible extraction is still performed inline after the final turn, not dispatched to a durable background job; move it only when live latency/cost data justifies that additional infrastructure.
2. **Historical finding — closed by ADR-030.** Chat replay is cursor-paginated and older canonical rows remain user-accessible. Prompt context uses a persisted deterministic summary plus the newest 12 messages, rather than silently dropping history. The remaining limitation is fidelity: deterministic excerpts are intentionally cheaper than LLM summarization and should be tuned against real traffic before introducing an optional background LLM-summary path.
3. **Completion is the only durability boundary.** A client disconnect, provider failure, or cancelled stream before a terminal event persists neither the user message nor an incomplete assistant response. This preserves clean transcripts, but loses user intent and makes retry/recovery/auditing incomplete. Decide whether to record a pending/failed turn before streaming and finalize it transactionally.
4. **The Chat history and title paths lack a real external-provider/database end-to-end test.** The focused suite validates route behavior with fakes; it does not prove title generation plus durable replay against an actual database and Groq. The project’s prior LangSmith/S3 incidents show why this deserves a controlled staging smoke test.
5. **Per-user generation cost now has a durable source of truth.** `generation_usage` is an append-only PostgreSQL ledger keyed by `request_id`, recording owner, generation/conversation/session IDs, provider/model/runtime, token counts, estimated USD cost, streaming and cache state. Both Generation and Streaming services write it after successful completion; cache replays are recorded at zero new-provider cost. `GET /usage/summary` is owner-scoped and distinguishes answer generation from `memory_extraction`, including memory-extraction cost per 100 answer turns. Historical usage intentionally remains unavailable: it was never persisted with an owner-linked cost. Streamed token usage can still be locally estimated where providers do not expose usage metadata, so this supports spend management—not invoice reconciliation.

These are additive findings, not a recommendation to revert the current design. The immediate priority remains a bounded query-rewrite + retrieval design for Chat; the operational items above prevent that new capability from inheriting avoidable cost, durability, and scaling problems.
...
- **Historical finding — superseded by the 2026-07-19 re-audit.** `app/ai/quality/` remains scaffold-like, but the repo-root `benchmarks/` package now provides real retrieval, reranking, generation, and regression evaluation. CI enforcement, broader end-to-end/security coverage, and production datasets remain open.
...
- **Historical finding — superseded by the 2026-07-19 re-audit.** The old `app/ai/quality/` scaffold remains empty, but it is no longer the evaluation source of truth: `benchmarks/` contains real benchmark and regression-detection code.
...
Beyond the RAG-wiring gap (now half-closed rather than fully open), the platform is still missing several "day 2" capabilities: **tracing has a real backend** (see §0/§4.12 — LangSmith, gated on explicit config, verified live against generation traffic), the broader `MetricsRecorder` interface remains `NoOp`, and benchmark coverage exists but is not enforced in CI or broad enough for security/end-to-end quality assurance. Domain exceptions still do not participate in the app's structured-error-response machinery, and production JSON logging remains a live concern. Agentic-flow readiness (LangGraph/MCP) remains 0% started.
...
|---|:-:|---|
| RAG / retrieval pipeline | 4/5 → **4/5** | Unchanged in capability (still a fully-real 4-provider compression stage, hybrid search + reranking); reachability is scored separately below and moved a lot. *Post-audit addendum, later session:* Parent/Child retrieval gained its missing producer (Hierarchical chunking) and Parallel Retrieval grew from 2-way to 3-way (dense+sparse+metadata) — see §4.1 addendum. Not re-scored, since this was a targeted fix verified by direct read, not a full re-audit pass |
...
| Cost tracking & token optimization | 3/5 → **3.5/5** | Owner-scoped PostgreSQL usage is now separated into answer and `memory_extraction` categories, zero-cost cache replays are recorded, and `/usage/summary` exposes memory cost per 100 answer turns. Held below 4/5 because streamed usage may be estimated, there are no budget alerts/limits, and representative live-traffic cost targets are not yet established. |
...
| Conversation memory — Chat | 3/5 → **3.5/5** | Persisted owner-scoped history/replay is now server-backed, terminal-event persistence is reliable, and replay order is deterministic. Still flattened into one prompt, bounded to 50 messages, and missing query rewriting/retrieval/citations (§0.0). |
| Conversation memory — Research | *(not previously scored separately)* → **3.5/5** | The prior 0/5 finding is superseded by the 2026-07-19 addendum: `research_conversations`, session threading, server replay, transcript folding, and Memory Platform context are live. Query rewriting/condensation and native message arrays remain future work. |
| Durable-memory lifecycle | *(not previously scored separately)* → **4/5** | The original architecture is now cost-aware: compact SESSION state, availability short-circuit, one shared embedding, parallel/fail-open durable retrieval, policy-gated/idempotent extraction, bounded interest promotion, provider fallback, and structured metrics are live. Held below 5/5 pending representative traffic SLOs and an explicit decision on durable background processing. |
...
| Agentic-flow readiness (LangGraph/MCP/tools/memory) | 0.5/5 → **0.5/5** (2026-07-18) → **2.5/5** (2026-07-23) → **3.5/5** (2026-07-23, later) → **3.75/5** (2026-07-25/26) | The linear Research API itself is still exactly as described for 07-18 — unchanged, still no decomposition/planning/agents on that path, by design. The Research Runtime (§0.0.0/§0.0.0a) is a real LangGraph `StateGraph` single-agent workflow — planning (now memory-aware, with query rewriting), decomposition, parallel fan-out, bounded repair loops, and a genuine second human-in-the-loop checkpoint (report-approval `interrupt()`, not just plan approval) — with a full working frontend as of the later 07-23 pass. **07-25/26 (§0.0.0b)**: a third `interrupt()` checkpoint (Web Search, ADR-036) and ResearchMind's first MCP client (Paper Search, ADR-037) both landed. Held at 3.75/5, not higher: both are narrow, hard-coded, single-external-service integrations, not a general orchestration layer or MCP ecosystem — no MCP manager/registry/discovery/routing (Phase 7 remains 0%), no generic tool-execution loop driving the provider-level `request.tools` plumbing, and the two new tool platforms are additive capability bolted onto the existing single-agent design, not evidence of a move toward multi-agent orchestration (ADR-033's decision criteria are unaffected); ~~single-worker, not horizontally scaled~~ (§0.0.0a item 3's residual gap — **fixed 2026-07-24, see item 27 below**: configurable in-process worker concurrency + global load-shedding on approval; still static config, not autoscaling). |
...
Unchanged from the original audit in every respect that matters: upload, Docling parsing, four chunking strategies (Fixed/Recursive/Markdown/Hierarchical — Hierarchical shipped in a later session than this audit's own revisions, see below), three embedding providers with a real Valkey cache, hybrid retrieval with genuine RRF fusion, two reranking providers, and the full context-assembly pipeline (including the fully-real, 4-provider compression stage) are all real. **What changed this cycle isn't the pipeline itself, it's that something finally calls it.**

**Post-audit addendum (later session, not a full re-audit):** two items this section previously flagged as gaps have since closed, verified by direct code read rather than a fresh end-to-end audit pass. First, **Parent/Child retrieval — previously consumer-only.** `ParentExpansionService` (§4.1's context-assembly pipeline) existed and was correctly wired into `ContextBuilderService.build()`, but nothing in the codebase ever set `structure.parent_chunk_id` on any chunk — the `ChunkingStrategy.HIERARCHICAL` enum value was reserved but unimplemented, so the expansion logic was dead code in practice. A new `HierarchicalChunkingProvider` (`chunking/providers/hierarchical.py`) now produces it: a two-pass LangChain `RecursiveCharacterTextSplitter` splits documents into parent sections, then splits each parent into child chunks; children carry `structure.parent_chunk_id`, and parents are tagged `is_parent` in `experiment.additional_metadata` and excluded from embedding/indexing by `EmbeddingService` (only children are searchable), but persist in the `ChunkArtifact` for the expansion service to resolve against. Second, **Parallel Retrieval — was dense+sparse only.** `RetrievalService.search_hybrid()`'s `asyncio.gather()` grew from 2-way to 3-way: a new `QdrantRetrievalProvider.search_metadata()` (filter-only Qdrant `scroll()`, no vector similarity, short-circuits to empty with no filters to avoid ignoring `owner_id` tenant scoping) is now fused in by RRF alongside dense and sparse. Neither change touches the report's central P0 finding (`chat.py` still hardcodes an empty `PromptContext`) or any other section's scoring — this addendum only corrects the two specific factual claims above and in the RAG/retrieval pipeline scorecard row footnote below.
...
This historical finding is superseded in part by the 2026-07-19 re-audit: the repo-root `benchmarks/` package now provides retrieval, reranking, generation, citation-accuracy, cost, and regression-detection coverage. The remaining gap is not "no evaluation harness"; it is that benchmarks are not a CI gate, datasets remain small/hand-curated, and the empty `tests/evaluation/`/`tests/security/` scaffolds still do not provide adversarial or end-to-end coverage.
...
> **Corrected 2026-07-25/26 (§0.0.0b).** "No MCP implementation" above is now false in the narrowest sense, not stale in the broad one: `app/ai/tools/paper_search/` (ADR-037) is a real MCP **client** — the official `mcp` SDK's `streamablehttp_client`, ResearchMind's first MCP code of any kind. It is a single, hard-coded integration to one external server for one tool (`search_papers`), with no client abstraction, registry, discovery, or routing — it is not an instance of the general-purpose MCP Ecosystem this section and the roadmap's Phase 7 describe, and does not move that gap. Separately, `app/ai/tools/web_search/` (ADR-036) added a genuine tool-execution path (Tavily search) driven by an agent-side necessity decision rather than the provider-level `request.tools` plumbing this paragraph refers to — a different mechanism reaching a similar outcome (the model's answer reflects a tool call it made), still not the generic `request.tools`-driven loop this finding originally asked for.
...
### 4.13 Conversation Memory — historical finding, superseded for Research

> **Superseded on 2026-07-19.** The narrative below records the pre-follow-up state. Research now has conversation IDs, persisted/replayable threads, transcript folding, and Memory Platform context. Its unresolved issue is query rewriting/condensation, not the absence of conversation memory. Chat's provider-message flattening remains current; see §0.0 for the current Chat audit and loopholes.
...
2. **Add bounded query rewriting/condensation before retrieval.** Both Chat and Research now have persisted history/memory, but neither resolves follow-up references into a standalone retrieval query. This is the missing stage in the intended conversational flow and should be designed with explicit user/session/semantic-memory boundaries, observability, and evaluation rather than by concatenating more transcript text.
...
6. ~~**Timeout plumbing for Gemini and Ollama**~~ **Partially closed, re-verified 2026-08-12** (`docs/IMPLEMENTATION_GAP_CROSSCHECK_2026-08-12.md`, engineering hygiene backlog). `OllamaProvider` now passes `timeout=config.timeout_seconds` to its client — fixed. `GeminiProvider`'s `genai.Client(api_key=...)` still takes no timeout argument at all — still open, now Gemini-only. See `docs/PRIORITIZED_ROADMAP.md`'s "Engineering hygiene backlog" section.
...
9. **Make the benchmark suite an enforceable quality gate.** Retrieval/reranking/generation/regression benchmarks now exist, but they are not yet a required CI/staging gate; datasets are small and hand-curated, and adversarial security/end-to-end evaluation remains absent. Add stable baselines, provider-aware variance handling, and a reviewable release threshold before using benchmark results for deployment decisions.
10. **Budget controls and staged usage validation.** Owner-scoped cost aggregation is now live in `generation_usage` and `/usage/summary`, including a separate `memory_extraction` category. Add configurable per-user/per-tenant budgets and alerts, then validate real extraction skip/empty rates, P50/P95 memory latency, and memory-extraction cost per 100 turns from a representative staging/production window.
...
12. **Harden the remaining Chat growth controls.** Title creation is lease-based and one-time; replay pagination and deterministic context compaction are now implemented. Decide how to represent interrupted streams, tune page/prompt-summary limits from real traffic, and use extraction-tail latency before deciding whether a durable background job is warranted.
...
16. **MCP** — ~~still zero code~~ **partially resolved 2026-07-25/26, not removed from this list so the historical number stays stable.** `app/ai/tools/paper_search/` (ADR-037) is a real MCP client — one external server, one tool, two call sites, no manager/registry/discovery/routing. This is not the general-purpose MCP Ecosystem the roadmap docs' Phase 7 plan describes, which remains genuinely at zero code; see §4.10's 2026-07-25/26 correction and §0.0.0b.
...
18. **L3 Session Cache and Session Artifacts wiring** — both implemented, still nothing calls either, since no session concept distinct from `Conversation` exists yet.
...
24. ~~**No proposal rejection/expiry, and no plan-edit-before-approval.**~~ **Half-stale, split 2026-08-12 per `docs/IMPLEMENTATION_GAP_CROSSCHECK_2026-08-12.md`.** Plan-edit-before-approval **shipped 2026-07-24** (D4, `PRODUCT_FLOWS_AND_GAPS.md`): a new `await_plan_approval` LangGraph `interrupt()` node between `aggregate`/`synthesize` gives the plan-approval checkpoint view/edit/reject-with-reason treatment, including `GET /research/runs/{id}/plan` and `POST /research/runs/{id}/plan-decision`. **Still genuinely open:** proposal-level rejection/expiry *before a run exists at all* — a proposal today can only be approved or left to expire per TTL, with no explicit "decline this proposal" endpoint or scope/priority adjustment prior to committing to a run. This is a separate, earlier stage than the plan-approval checkpoint that's now fixed, and doesn't have a `PRIORITIZED_ROADMAP.md` line item — see `docs/IMPLEMENTATION_GAP_CROSSCHECK_2026-08-12.md` Table B.
...
27. ~~**New (§0.0.0a item 3's residual gap)**: rate limiting is per-owner, not global — many different accounts can still each queue Deep Research approvals that pile up behind the single serial worker.~~ — **Fixed 2026-07-24 (D2, `REMAINING_WORK.md`)**: `research_runtime_main.py` now runs multiple concurrent claim lanes (`research_runtime_worker_concurrency`, each its own DB session; the outbox's `SELECT ... FOR UPDATE SKIP LOCKED` already made this safe), and `POST /proposals/{id}/approve` now enforces a genuinely global cap (`deep_research_max_queued_runs`) via `ResearchRunDispatchRepository.count_active()`, returning `503`+`Retry-After` once saturated. Residual: both are static config, not autoscaling, and the global cap is total-depth, not per-owner fair-share.
