...
| Tool-invocation rate & success rate (web/paper search) | ✓ (if toggled) | — | Excluded (see below) | **Done for Chat, 2026-08-12 (E23).** `GenerationRequest.metadata`/`eval_scores`/automatic LangSmith sync/existing dashboard view — no new judge, exactly the cheap count this row originally scoped. Deep Research explicitly excluded: `research_run_events` records web-search *outcomes* but not the *decision not to search* (no durable event there), so a true invocation rate isn't computable without a LangGraph node change, deliberately not attempted this pass; paper search has no Deep Research event type at all. Whether the *right* tool was chosen (vs. just "was it invoked and did it return something") remains the Mature-tier version below. |
