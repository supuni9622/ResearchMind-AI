...
| 2 | Memory production hardening + Projects isolation — M0-M2/M4-M11 implemented; rollout/calibration remains | High | Med | M3's recurring bounded worker remains report-only pending staging evidence. M6-M8 quality/consolidation work remains calibration-gated. M9-M10 add complete-history topical lookup and versioned typed preferences with deterministic safe-key supersession. M11 adds bounded storage/drift/lifecycle/utility operations visibility. M12-M16 remain sequenced in [`MEMORY_PLATFORM_PRIORITIZED_TASKS.md`](MEMORY_PLATFORM_PRIORITIZED_TASKS.md). |
