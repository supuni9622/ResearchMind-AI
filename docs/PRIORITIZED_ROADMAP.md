...
| 1 | ✅ CI live-service benchmark triggers + citation-metric wiring — Done, `fabricated_citation_rate` populated and verified live; `schema_validity_rate`/`abstention_pass_rate` deliberately still open (need separate design decisions, see E20) | High | Med | Gap-closure follow-up to E2/E4 — the absolute regression gates E2 declared are still never populated by any benchmark run |
