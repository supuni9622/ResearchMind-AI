...
| 2 | Memory production hardening + Projects isolation — M0-M2/M4/M5 complete; M3 rollout and M6 staging calibration pending | High | Med | M3's recurring bounded worker is implemented report-only; staging/production rollout evidence remains. M6's dataset, live capture adapter, paired answer judge, persistence, provisional budgets, and deterministic CI gate are implemented; seeded staging evidence and calibration remain. M7-M16 stay sequenced in [`MEMORY_PLATFORM_PRIORITIZED_TASKS.md`](MEMORY_PLATFORM_PRIORITIZED_TASKS.md). |
