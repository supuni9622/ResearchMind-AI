...
| 2 | Memory production hardening + Projects isolation — M0-M2/M4-M9 implemented; rollout/calibration remains | High | Med | M3's recurring bounded worker remains report-only pending staging evidence. M6-M7 evaluation/correlation/feedback tooling and M8 consolidation are implemented but still require rollout calibration. M9 closes dormant USER-preference supersession with scope-safe topical historical lookup. M10-M16 remain sequenced in [`MEMORY_PLATFORM_PRIORITIZED_TASKS.md`](MEMORY_PLATFORM_PRIORITIZED_TASKS.md). |
