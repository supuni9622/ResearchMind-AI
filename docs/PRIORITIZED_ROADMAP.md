...
| 2 | Memory production hardening + Projects isolation — M0-M2/M4/M5 complete; M3 rollout and M6-M7 calibration pending | High | Med | M3's recurring bounded worker is implemented report-only; staging/production rollout evidence remains. M6 evaluation tooling is implemented. M7 generation correlation, explicit memory feedback, and opt-in sampled utility scoring are implemented; migration rollout and calibration remain. M8-M16 stay sequenced in [`MEMORY_PLATFORM_PRIORITIZED_TASKS.md`](MEMORY_PLATFORM_PRIORITIZED_TASKS.md). |
