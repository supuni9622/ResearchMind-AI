...
| ~~D5~~ | ~~Report-approval decision has no timeout~~ — **Fixed 2026-07-24** (commit `95a6c8c`, missed in earlier passes of this document — corrected now): `ResearchRunService.expire_stale_awaiting_approval()` auto-*cancels* (deliberately not auto-approve) any run left at `AWAITING_APPROVAL`, `AWAITING_PLAN_APPROVAL`, or `AWAITING_WEB_SEARCH_APPROVAL` past `settings.research_runtime_awaiting_approval_ttl_hours` (default 72h), with a distinct `terminal_reason` per state. Genuinely scheduled in production — piggybacked on `ResearchRuntimeWorker`'s existing poll loop (at most once/hour) rather than a separate cron process, wired via `app/bootstrap/worker.py::create_research_runtime_worker()`. Covers all three approval pauses, not just report approval. | Resolved |
