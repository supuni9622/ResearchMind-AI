...
| Milestone 12 — Memory Platform | ✅ Core complete; M0-M5 foundation implemented; M6 code complete with staging calibration pending — Session/User/Semantic/Research memory, `/memory` API, RRF search, LLM extraction, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, personal/project isolation, deterministic/live memory capture evaluation, paired answer utility, persistence, and release gates. M3 production rollout, M6 seeded staging evidence/calibration, and Project-product activation remain pending. |
