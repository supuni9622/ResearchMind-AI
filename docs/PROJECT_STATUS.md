...
| Milestone 12 — Memory Platform | ✅ Core complete; M0-M2/M4-M9 implemented with operational rollout pending — Session/User/Semantic/Research memory, `/memory` API/UI, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, personal/project isolation, deterministic/live evaluation, generation correlation, explicit feedback, sampled utility/harm scoring, reversible consolidation, and complete-history topical USER supersession. M3 production rollout, M6-M9 staging evidence/calibration, and Project-product activation remain pending. |
