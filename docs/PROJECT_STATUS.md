...
| Milestone 12 — Memory Platform | ✅ Core complete; hardening M0-M5 foundation implemented — Session/User/Semantic/Research memory, `/memory` API, RRF search, LLM extraction, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, and personal/project isolation. M3 production rollout and Project-product activation remain pending. |
