...
**Current addendum (2026-08-17):** M9-M10 USER-preference quality is implemented: scope-safe topical lookup searches complete authorized history, and versioned typed attributes allow deterministic replacement only for one unique, explicit, high-confidence controlled key; all uncertain cases retain audited conservative judgment. M11 adds scheduled bounded PostgreSQL/Qdrant inventory, drift and freshness alerts, and dashboard coverage for storage growth, lifecycle, context budgets, throttling, consolidation, and utility. Memory M0-M2/M4-M11 are implemented; M3 production rollout and M6-M10 staging calibration/rollout gates remain. The same verification pass fixed Linear Research false relevance: owner-only metadata scrolling no longer injects arbitrary chunks, reranking evaluates the expanded pool, an absolute configurable relevance gate can return zero documents, and the UI no longer labels every weak best match as 100%.
...
| Milestone 12 — Memory Platform | ✅ Core complete; M0-M2/M4-M11 implemented with operational rollout pending — Session/User/Semantic/Research memory, `/memory` API/UI, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, personal/project isolation, quality signals, reversible consolidation, complete-history topical USER supersession, versioned typed preferences, and bounded storage/drift/lifecycle/utility dashboards and alerts. M3 production rollout, M6-M10 staging evidence/calibration, and Project-product activation remain pending. |
...
✅ Personal Memory Management Slice (partial M12/M13) — added a first-class
