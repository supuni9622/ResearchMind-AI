...
| Milestone 12 — Memory Platform | ✅ Core complete; M0-M5 foundation implemented; M6 in progress — Session/User/Semantic/Research memory, `/memory` API, RRF search, LLM extraction, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, personal/project isolation, and the first deterministic offline memory benchmark slice. M3 production rollout, M6 live capture/answer utility, and Project-product activation remain pending. |
