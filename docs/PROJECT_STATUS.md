...
| Milestone 12 — Memory Platform | ✅ Core complete; M0-M7 implementation present with operational rollout pending — Session/User/Semantic/Research memory, `/memory` API, RRF search, LLM extraction, runtime injection, recurring dry-run lifecycle worker, coordinated prompt token budgeting, personal/project isolation, deterministic/live evaluation, generation-to-memory correlation, explicit memory feedback, and opt-in sampled utility/harm scoring. M3 production rollout, M6-M7 staging evidence/calibration, and Project-product activation remain pending. |
