...
**Current addendum (2026-08-17):** Memory M0-M2/M4-M16 are implemented, including scope-aware management, Personal/Project UI, portable export, server-confirmed cross-store erasure, prompt delimiters, quotas, and failure contracts. M14-M15 destructive governance requires Alembic revision `c8d9e0f1a2b3`; a live frontend verification found and corrected one database that was still at `b7c8d9e0f1a2`, plus added a bounded preview timeout and inline retry UX. M3 production rollout evidence and M6-M10 calibration remain. The same verification pass fixed Linear Research false relevance: owner-only metadata scrolling no longer injects arbitrary chunks, reranking evaluates the expanded pool, an absolute configurable relevance gate can return zero documents, and the UI no longer labels every weak best match as 100%.
...
| Milestone 12 — Memory Platform | ✅ M0-M2/M4-M16 implemented with operational rollout pending. M3 evidence, M6-M10 calibration, and broader Project-product activation remain. |
