...
│   │       │   │   ├── observability/         # Structured operation/quality metrics plus M11 scheduled bounded PostgreSQL/Qdrant inventory and drift collection
...
│   │       │   │   │   └── chat/models.py          # ChatEventType — chat_web_search_started/completed/skipped, category=TOOL (new, 2026-07-25, ADR-036)
│   │       │   │   ├── chat/                       # Chat's toggle-gated, no-approval web search step (new, 2026-07-25, ADR-036)
│   │       │   │   │   └── web_search.py          # run_chat_web_search() — reuses ai/tools/web_search/ + ai/runtime/research/web_search/ unchanged; best-effort, degrades to no search on any failure
...
│   │       │   │       ├── execution.py          # ResearchRuntimeExecutionService — execute_approved_run() (the real worker-driven path: resume-aware, budget/recursion-limit-bounded, classified exception handling incl. report rejection); execute() (sync in-request path, zero production callers). _finalize_or_pause()/_resume_v1_graph_after_report_approval() added 2026-07-23 later for the report-approval interrupt; _resume_v1_graph_after_web_search_approval() added 2026-07-25 for the web-search-approval interrupt (ADR-036)
│   │       │   │       ├── web_search_inspection.py  # ResearchWebSearchInspectionService — reads the pending web-search suggestion straight out of the paused run's checkpoint, mirrors plan_inspection.py/draft_inspection.py (2026-07-25, ADR-036)
│   │       │   │       ├── web_search/            # Research-Runtime glue for web search (ADR-036, 2026-07-25): models.py (WebSearchMode, WebSearchNecessityDecision), necessity.py (WebSearchNecessityService — cheap OpenAI/Claude structured decision, deterministic DISABLED/REQUIRED pre-rules), evidence.py (normalize_web_search_result — Context Guardrails prompt-injection scan, produces ResearchEvidenceReference(source_type="web")), create.py (dedicated cheap-model registry, separate from the shared GenerationRegistry)
...
│   │       │   │       └── workflows/            # multi_wave_research.py (the real production StateGraph — Send() parallel fan-out, budget-aware bounded repair routing, cooperative cancellation checks, a real interrupt()-based await_report_approval node added 2026-07-23 later; evaluate_web_search_need/await_web_search_approval/search_web_gap added 2026-07-25 inside the existing bounded gap-research loop, ADR-036), task_research.py (single-task variant)
│   │       │   ├── tools/                   # Web Search Tool Platform (ADR-036, 2026-07-25) — framework-independent, no LangGraph/Research Runtime imports
│   │       │   │   └── web_search/              # enums.py, models.py (WebSearchRequest/Result), interfaces.py (WebSearchProviderInterface), policies.py, exceptions.py, registry.py, service.py (WebSearchService), create.py; providers/tavily.py (raw httpx, no SDK dep), providers/fake.py (tests)
...
│   │   │   │   │   ├── chat/
│   │   │   │   │   │   └── page.tsx         # Server-backed Chat conversation interface; gained an opt-in web search toggle (2026-07-25, ADR-036) — Chat's only grounding source today, no document retrieval. Logic lives in src/features/chat/ (use-chat.ts, components/chat-composer.tsx, components/message-bubble.tsx, types.ts) — not itemized below, same pre-existing gap as src/features/research/ noted below; see FILES.md
│   │   │   │   │   ├── research/
...
| Retrieval Platform | `apps/api/app/ai/knowledge/retrieval/` | Queries the hybrid Qdrant index with dense, sparse (SPLADE), RRF fusion, and metadata filtering only when a real document constraint is present; owner scope alone is access control and never an unranked retrieval vote. The expanded fused pool is reranked, real cross-encoder relevance scores replace rank-only RRF scores, and `RETRIEVAL_RERANK_SCORE_THRESHOLD` can return zero irrelevant chunks. `/retrieve`, `/retrieve/sparse`, and `/retrieve/hybrid` remain auth-protected and server-scoped to `owner_id` — ADR-018, ADR-019, ADR-020, ADR-021. |
...
| Memory Platform | `apps/api/app/ai/memory/` | Provides compact SESSION state, canonical PostgreSQL USER/SEMANTIC/RESEARCH memory, Qdrant indexing, and bounded injection for Chat and Research. M0-M2/M4-M11 are implemented: bounded mutations/extraction, coordinated prompt budgets, project-safe storage/authorization, quality signals, reversible consolidation, complete-history topical USER supersession, typed preferences, and bounded storage/drift/lifecycle/utility operations visibility. M3 rollout and M6-M10 staging calibration remain. See `docs/architecture/memory-platform.md` §26 and ADR-029. |
