...
| Memory Platform | `apps/api/app/ai/memory/` | Provides compact SESSION state, canonical PostgreSQL USER/SEMANTIC/RESEARCH memory, Qdrant indexing, and bounded injection for Chat and Research. M0-M2/M4-M9 are implemented: bounded mutations/extraction, coordinated prompt budgets, project-safe storage/authorization, offline and online quality signals, reversible SEMANTIC/RESEARCH consolidation, and complete-history topical USER supersession with conservative audited decisions. M3 rollout and M6-M9 staging calibration remain. See `docs/architecture/memory-platform.md` §26 and ADR-029. |
