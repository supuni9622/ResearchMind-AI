...
| Memory Platform | `apps/api/app/ai/memory/` | Provides compact SESSION state, canonical PostgreSQL USER/SEMANTIC/RESEARCH memory, Qdrant indexing, bounded injection, and the M12 scope-aware management API with safe projections, explicit edits/moves, settings, and cross-tenant tests. M0-M2/M4-M12 are implemented; M3 rollout, M6-M10 calibration, and M13 Project UI remain. See `docs/architecture/memory-platform.md` §26 and ADR-029. |
