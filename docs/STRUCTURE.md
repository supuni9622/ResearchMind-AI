...
| Memory Platform | `apps/api/app/ai/memory/` | Provides compact SESSION state, canonical durable memory, Qdrant indexing, bounded untrusted injection, scope-aware management UI/API, portable export, confirmed erasure jobs, quotas, and drift inventory. M0-M2/M4-M16 are implemented; M3 rollout and M6-M10 calibration remain. See `docs/architecture/memory-platform.md` §26 and ADR-029. |
