...
| Retrieval-path metrics | **Strongly aligned** | Durable availability, empty/skip state, parallel search, semantic/research searches, and loaded session items are tracked. M6 now includes an offline Recall@5/Precision@5/MRR/nDCG scorer. | Live staging capture and calibrated regression baselines remain. |
...
| **P0, in progress (M6)** | Complete the memory evaluation dataset and regression suite | The versioned seed dataset, deterministic scorer, and zero-tolerance scope/unsafe gates are implemented; live capture and answer utility remain. | Live captured retrieval, paired memory-on/off answers, calibrated budgets, and CI/staging enforcement. |
