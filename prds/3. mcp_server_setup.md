...
to `http://127.0.0.1:8080/mcp`. Add an `Authorization: Bearer <token>`
...
```bash
docker run --rm -p 8000:8080 --env-file .env research-intelligence-mcp:local
```

Endpoint: `http://127.0.0.1:8080/mcp` (or `http://research-intelligence-mcp:8080/mcp`
...
```bash
curl -s http://127.0.0.1:8080/health    # liveness — always unauthenticated
curl -s http://127.0.0.1:8080/ready     # readiness — 503 once shutting down
curl -s http://127.0.0.1:8080/metrics   # Prometheus text format — keep private
```
